...
function penata_main($arg=NULL, $nama=NULL) {
	...
	$query = db_select('dokumen' . $tahun, 'k')->extend('PagerDefault')->extend('TableSort');
	$query->innerJoin('unitkerja' . $tahun, 'u', 'k.kodeuk=u.kodeuk');

	# get the desired fields from the database
	$query->fields('k', array('dokid','kodeuk', 'sp2dno','sp2dtgl','keperluan', 'kegiatan', 'penerimanama', 'jumlah'));
	...
	if ($jenisdokumen !='##') $query->condition('k.jenisdokumen', $jenisdokumen, '=');
...
function penata_main_form($form, &$form_state) {
	...
	$form['formdata'] = array (
		'#type' => 'fieldset',
		'#title'=> '<p>' . $tahun . $namasingkat . $jenisdokumen_label . $bulan_label .  '</p>' . '<p><em><small class="text-info pull-right">klik disini utk menampilkan/menyembunyikan pilihan data</small></em></p>',
	...
	$form['formdata']['skpd'] = array(
		...
		'#options' => _ajax_get_skpd_dropdown($tahun),
