<?php
function laporanapbd_perubahan_main($arg=NULL, $nama=NULL) {

drupal_set_title('APBD Perubahan ' . apbd_tahun());
//$output_form = drupal_get_form('laporanapbd_main_form');	
$output_form = laporanapbd_perubahan_main_form();	
return drupal_render($output_form);
}


function laporanapbd_perubahan_main_form() {
...
$tsd_pembiayaan_netto = 0; 
	...
	$form['menu1']['m11'] = array(
		...
		'#type' => 'fieldset',
		'#title'=>  _bootstrap_icon('unchecked') . ' Penundaan APBD Perubahan<em><small class="span4 text-info pull-right">' . $label_milyar  .  '</small></em>',
		'#collapsible' => FALSE,
		'#collapsed' => FALSE,		
		'#suffix' => '</div>',
	);	
...
//SURPLUS/DEFISIT
$tsd_surplus = $agg_pendapata_total - $tsd_belanja_total;
...
$form['menu1']['m11']['tab11']['rowkmltf_sd']= array(
	'#prefix' => '<td valign="center" style="border-top:1px solid black">',
	'#type'         => 'item', 
	'#markup' => '<strong><p align="right">' . apbd_fn(($agg_surplus-$tsd_surplus) / $sejuta) . '</p></strong>', 
	'#suffix' => '</td>',
);
...
$form['menu1']['m11']['tab11']['rowuraian_bD']= array(
	...
	$tsd_silpa = $tsd_surplus + $agg_pembiayaan_netto;
