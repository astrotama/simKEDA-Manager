...
function laporanperencanaan_detiluk_main($arg=NULL, $nama=NULL) {
	...
	drupal_set_title($skpd);
	
	
	$output = gen_data($kodeuk, $page);
		
	return drupal_render($output);		
	//return $output;
}

function gen_data($kodeuk, $page) {
...
$total_ppa = $_SESSION["perencanaan_ppas" . $kodeuk];
$total_agg = $_SESSION["perencanaan_apbd" . $kodeuk];
...
foreach ($query as $data) {
	$jumlahpage = ceil($data->jumlahkeg / apbd_row_limit());
	$jumlahkeg = $data->jumlahkeg;
}

$results = db_query('select kodekeg, idrenja, kegiatan, total from {kegiatanskpd} where jenis=2 and total>0 and kodeuk=:kodeuk order by kegiatan limit ' . $start . ', ' . apbd_row_limit(), array(':kodeuk'=>$kodeuk));	
...
foreach ($arr_result as $data) {
	...
	$kegiatan = l($data->kegiatan, 'laporanapbddetilkeg/filterkegppa/' . $kodeuk . '/' . $data->kodekeg, array('attributes' => array('class' => null)));
	...
	$form['m11']['tab11']['row15' . $i]= array(
		...
		'#markup' =>  apbd_simbol_perencanaan($ppa, $data->total),
		'#suffix' => '</td></tr>',
	);	
